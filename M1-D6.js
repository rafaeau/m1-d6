...
splitMe = function (stringToSplit) {
  return stringToSplit.split(" ")
}
console.log(splitMe("Rafael Neves Ferreira"))
...
const onlyInThisMillennium = function (array) {
  movies2000=[]
  for (let i=0; i<array.length; i++) {
    if (array[i].Year >= 2000) {movies2000.push(array[i].Title, array[i].Year)}
  } return movies2000
}
...
const getMovieById = function (idSearch) {
  searchResult=[]
  for (let i=0; i<movies.length; i++) {
    if (movies[i].imdbID === idSearch) {searchResult.push(movies[i])}
  } return searchResult
}
...
const sumAllTheYears = function (array) {
  let moviesYears = 0
  for (let i=0; i<array.length; i++) {
    moviesYears += parseInt(array[i].Year)
  } return moviesYears
}
...
const searchByTitle = function (search) {
  searchResult=[]
  for (let i=0; i<movies.length; i++) {
    if (movies[i].Title.toLowerCase().includes(search)) {searchResult.push(movies[i])}
  } return searchResult
}
